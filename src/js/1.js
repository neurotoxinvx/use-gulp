...
self._debug("change direction to "+(direction?"next":"prev"));nowDirection=direction;}}).mouseout(function(){nowDirection=null;self._debug("clear direction");}).click(function(event){var direction=(event.pageX-jQuery(this).offset().left)/jQuery(this).width()>0.5;if(direction){self.next();}else{self.prev();}}).load(function(){var img=self.screen.get(0);var height="";var width="";self.screen.css("height","").css("width","");self._debug("pic:"+img.width+"x"+img.height);if(parseInt(img.width)>500){width=935;height=609;}else{if(parseInt(img.height)>1000){width=parseInt((500/img.height)*img.width);height=500;}}
